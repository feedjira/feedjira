  ...
  describe ".parse" do
    context "when the parser is specified" do
      it "parses an rss feed" do
        ...
        published = Feedjira::TimeParser.parse_safely "Thu Dec 04 17:17:49 UTC 2008"
    ...
    context "when there's an available parser" do
      it "parses an rdf feed" do
        ...
        published = Feedjira::TimeParser.parse_safely("Tue Sep 02 19:50:07 UTC 2008")
        expect(feed.entries.first.published).to eq published
        expect(feed.entries.size).to eq 10
      end
      ...
      it "parses an feedburner atom feed" do
        ...
        published = Feedjira::TimeParser.parse_safely "Thu Jan 22 15:50:22 UTC 2009"
        expect(feed.entries.first.published).to eq published
      ...
      it "parses an itunes feed" do
        ...
        published = Feedjira::TimeParser.parse_safely "Wed, 15 Jun 2005 19:00:00 GMT"
        expect(feed.entries.first.published).to eq published
      ...
      it "parses an itunes feedburner feed" do
        ...
        published = Feedjira::TimeParser.parse_safely "2023-09-22 16:30:15 UTC"
        expect(feed.entries.first.published).to eq published
        expect(feed.entries.size).to eq 3
      ...
      it "does not fail if multiple published dates exist and some are unparseable" do
        ...
        published = Feedjira::TimeParser.parse_safely "Mon, 16 Oct 2017 15:10:00 GMT"
        expect(feed.entries.first.published).to eq published
        expect(feed.entries.size).to eq 2
      end
    end
    ...
    it "parses an feedburner rss feed" do
      ...
      published = Feedjira::TimeParser.parse_safely "Wed Nov 02 17:25:27 UTC 2011"
      expect(feed.entries.first.published).to eq published
    ...
    it "parses an RSS feed with an a10 namespace" do
      ...
      expect(feed.entries.first.updated).to eq Feedjira::TimeParser.parse_safely("2020-05-14T10:00:18Z")
