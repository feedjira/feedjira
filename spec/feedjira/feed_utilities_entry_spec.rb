...
describe Feedjira::FeedUtilities do
  ...
  describe "handling dates" do
    it "parses an ISO 8601 formatted datetime into Time" do
      ...
      expect(time).to eq Feedjira::Utils.parse_time_safely("Wed Feb 20 18:05:00 UTC 2008")
    end
  ...
  describe "sanitizing" do
    ...
    it "provides a sanitized title" do
      ...
      expect(Feedjira::Utils.sanitize_string(@entry.title)).to eq scrubbed_title
    end

    it "sanitizes content using Utils" do
      new_content = "<script>#{@entry.content}"
      @entry.content = new_content.dup

      scrubbed_content = Loofah.scrub_fragment(new_content, :prune).to_s

      expect(Feedjira::Utils.sanitize_string(@entry.content)).to eq scrubbed_content
    end
