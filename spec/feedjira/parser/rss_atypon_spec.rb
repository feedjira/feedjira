...
module Feedjira::Parser
  describe RSSAtypon do
    describe '#will_parse?' do
      it 'should return true for a atypon rss feed' do
        expect(RSSAtypon).to be_able_to_parse(sample_atypon_rss_feed)
      end

      it 'should return false for a feedburner rss feed' do
        expect(RSSAtypon).to_not be_able_to_parse(sample_rss_feed_burner_feed)
      end

      it 'should return false for a regular RSS feed' do
        expect(RSSAtypon).to_not be_able_to_parse(sample_rss_feed)
      end

      it 'should return false for a feedburner atom feed' do
        expect(RSSAtypon).to_not be_able_to_parse(sample_feedburner_atom_feed)
      end

      it 'should return false for an rdf feed' do
        expect(RSSAtypon).to_not be_able_to_parse(sample_rdf_feed)
      end

      it 'should return false for a regular atom feed' do
        expect(RSSAtypon).to_not be_able_to_parse(sample_atom_feed)
      end
    end

    describe 'parsing' do
      before(:each) do
        @feed = RSSAtypon.parse(sample_atypon_rss_feed)
      end
