  ...
  describe RSS do
    describe '#will_parse?' do
      it 'should return true for an RSS feed' do
        expect(RSS).to be_able_to_parse(sample_rss_feed)
      end

      it 'should return false for an atom feed' do
        expect(RSS).to_not be_able_to_parse(sample_atom_feed)
      end

      it 'should return false for an rss feedburner feed' do
        able = RSS.able_to_parse? sample_rss_feed_burner_feed
        expect(able).to eq false
      end

      it 'should return false for a atypon rss feed' do
        expect(RSS).to_not be_able_to_parse(sample_atypon_rss_feed)
      end
    end

    describe 'parsing' do
      before(:each) do
        @feed = RSS.parse(sample_rss_feed)
      end
      ...
      it 'should parse the image description' do
        expect(@feed.image.description).to eq 'The act of making love, tenderly.' # rubocop:disable Metrics/LineLength
      end
